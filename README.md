...
This example treats aggregation as a business choice: find the lowest matching offer in each source, keep the source visible, and turn the first result into a small order record. That shape is useful when a later fulfillment worker or customer update needs an unambiguous receipt rather than a loose scrape result.

The runnable path is deliberately small. `src/listing_service.py` owns the decision and the checkout/receipt transition; `src/example.py` supplies two in-memory shop feeds so the example can run without credentials. When semantic matching becomes useful, the same module also shows an Infrai embeddings request using one `INFRAI_API_KEY` and the OpenAI-compatible endpoint.
...
The focused test exercises the business decision (the cheapest matching offer per source), not merely an import:
...
Set `INFRAI_API_KEY` in the process environment before calling `create_embedding("canvas travel bag")`. The function sends `input` and `model` to `POST /v1/embeddings`, decodes the `{ok, data, error, metadata}` envelope first, and returns the vector for your own ranking step. Keeping that boundary explicit makes it easy to compare lexical filtering with semantic retrieval while the order model stays unchanged.

## Why this boundary

A scraper-shaped object can expose every source detail, but it leaves checkout logic scattered across callers. Here the listing is a typed value and the order is a plain dictionary with a visible status, total, and currency; that is enough for a fulfillment adapter or a customer update without inventing extra framework code.
